...
void PlottingCodeLoop::Loop()
{
  ...
  FRparams.push_back(0.04831);
  FRparams.push_back(2024.);
  FRparams.push_back(2.731);

  TString FRformula = TString::Format("(%f+(%f/x^(%f)))",FRparams[0],FRparams[1],FRparams[2]);
  cout<<"Fake rate function used: "<<FRformula.Data()<<endl;

  TF1* fake_rate_fn  = new TF1("fake_rate_fn",FRformula.Data(),10.,2000.) ;
  TF1* KFactorFunction = new TF1("KFactorFunction","1.07559+(1.222/(x^(-.00185612)))",10.,2000.);   
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if (_fakeStatus=="TightTight"){
      ...
      if (_SampleType == "mc"){                      
	PUweight = MCPUWeight;
	if (Diphoton_Minv>=320){KFactorweight = 1.;} 
	if (Diphoton_Minv<320){KFactorweight=1.;}
